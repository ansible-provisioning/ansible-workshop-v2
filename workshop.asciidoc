...
== Inventory Groups ==
The application we want to deploy is multi-tiered. There is the web server part and the database part. It makes sense to be able to run commands on all web servers.
...
In this case the ws01 virtual machine will have both roles.
...
TASK: [Install MySQL] ********************************************************* 
ok: [ws01] => (item=mysql-server,MySQL-python)
...
With `--limit` we can run a playbook on a subset of hosts.

Run the new playbook on this host:

----
[root@ws01 workshop]# ansible-playbook auto-deploy.yml -i hosts --limit=ws01
...
TASK: [Update SSH key permissions] ******************************************** 
ok: [ws01]
...
remote: changed: [ws01] => (item=/tmp/ws-deploy/app/web/index.php)
remote: 
remote: PLAY RECAP ******************************************************************** 
remote: ws01                       : ok=7    changed=2    unreachable=0    failed=0   
remote: 
To /root/deploy
